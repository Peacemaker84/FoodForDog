...
export default () => {
  ...
  const handler = (e) => {
    ...
    if (id) {
      api.modifyProduct(id, body).then((data) => {
        console.log(data);
        modifyData(data);
      });
    } else {
      api.addProduct(body).then((data) => modifyData(data));
    }
  };
  ...
  return (
    <div className="addForm">
      <Link
        ...
        to={id ? `${PATH}catalog/${id}` : `${PATH}catalog`}
      >
        <i className="fa-solid fa-angle-left" /> Назад
      </Link>
      <p className="form-header">{id ? "Изменить товар" : "Добавить товар"}</p>
